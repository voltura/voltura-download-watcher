...
- The Windows-facing assembly name is `Voltura Download Watcher`, producing `Voltura Download Watcher.exe`; keep installer, publish, screenshot, shortcuts, and startup-registration paths aligned with that spaced filename. The C# namespace, project directory, registry value name, mutex, and event names remain `VolturaDownloadWatcher`.
- The WPF window, Task Manager process group, and notification-area states come from the generated multi-resolution application ICO. Runtime active, paused, and update markers are composited over that common source.
